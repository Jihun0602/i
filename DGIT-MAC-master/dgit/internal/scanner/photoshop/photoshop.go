...
func createBasicLayersFromNames(layerNames []string) []DetailedLayer {
    layers := make([]DetailedLayer, len(layerNames))
    
    for i, name := range layerNames {
        // Generate a simple content hash based on name and position
        hash := sha256.Sum256([]byte(fmt.Sprintf("%s_%d", name, i)))
        ...
        layers[i] = DetailedLayer{
            ...
            ContentHash: fmt.Sprintf("%x", hash)[:16], // First 16 chars of hash
            LayerType:   "normal",                  // Default layer type
        }
    }
...
// parseDetailedLayers attempts to parse comprehensive layer information
// TODO: Implement full detailed PSD layer parsing for advanced analysis
func parseDetailedLayers(file *os.File, layerCount int) ([]DetailedLayer, error) {
    // For now, return error to use basic layer creation fallback
    // TODO: Implement detailed PSD layer parsing including:
    // - Layer blend modes and opacity
    // - Layer visibility states
    // - Layer content hashing for change detection
    // - Layer type identification (normal, text, adjustment, etc.)
    // - Precise layer bounds and positioning
    return nil, fmt.Errorf("detailed layer parsing not yet implemented")
}